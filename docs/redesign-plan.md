...
- [x] **How it works**: three steps, italic terracotta numerals, hairline dividers. Copy from the canvas's `STEPS`.
- [x] **For coaches CTA**: shade card (`#efe9dc`), radius 16/20, two concentric terracotta hairline circles top-right (desktop), ink pill button (hover terracotta). Copy: "$9.99 a month … No commission, ever."
- [x] Delete `hero.tsx`'s split-layout CSS blocks in `globals.css`; keep the safe-area work (`--safe-top`, `100dvh`, `viewport-fit=cover`, `<html>` background).

**Done 10 Sep 2026.** Hero, search card, marquee, coach card, discipline list/grid, steps and coach CTA rebuilt to the 1a frames; `wide:` (1100px) added as a Tailwind breakpoint for the canvases' phone/desktop switch. `/api/coach-count` powers the live "Show N near Town" CTA; `/api/location-suggest` now title-cases and ranks major towns first (plain alphabetical ranking offered "Bend Of Islands" before "Bendigo"). Discipline counts are real (published + mock). Three CSS lessons caught by the checker, not by eye: (1) an unlayered `h2 { letter-spacing }` rule silently beat every `wide:-tracking-[…]` utility — heading defaults now sit in `@layer base`; (2) Tailwind v4 wants `-tracking-[0.025em]`, not `tracking-[-0.025em]`; (3) a `display:none` fallback written after its `@media` override wins by source order. The search card was first built as two responsive copies of the fields — replaced with one set reordered by flex `order` so the DOM has one input. Measured: **74/74** assertions at 390 and **49/49** at 1280 (`assertions/r2-home-*.json`); `home-behaviour.mjs` all green — reveal blocks start at opacity 0 / 20px and flip on entry, parallax = 0.28×scrollY capped 260 at three offsets, wordcycle steps dressage → western → liberty at 1/3/5s, "Bend" → Bendigo VIC chip → CTA "Show N near Bendigo" with N equal to the API, re-queried on discipline change, submit lands on `/search?location=Bendigo+VIC&d=dressage`; no horizontal scroll and search card above the fold at 320×568 … 2560×1440; reduced motion collapses every animation and shows one static word; no-JS content visible; exactly one hero image fetched, AVIF, `fetchpriority=high`. Contrast (worst painted pixel): eyebrow 8.20 / 5.21, h1 13.27 / 14.71, lead 12.53 / 13.04, stats 15.80 (desktop / phone). Old `band-*` hero files and the masonry/tag components removed.
...
- [x] **Ink header variant** with the search summary pill ("Bendigo VIC · Dressage · within 50 km" + "Edit"). "Edit" opens the homepage search card in a sheet (mobile) / inline popover (desktop) pre-filled — reusing the R2 component.
- [x] Mobile: chips row under the header (horizontal scroll, `.hs`), "N coaches" h1 + "Nearest first", "Within N km of Town", radius `<input type=range>` (accent terracotta), card list, "Nobody quite right?" ink card with "Notify me" (→ `/account#alerts` with the search's location and discipline pre-filled, or `/signup?role=rider&next=…` when logged out). **Floating list/map toggle** pill, sticky bottom, ink with a peach dot.
- [x] Desktop: chips row + radius slider on the right; h1 "N coaches *near Town*"; **2-column card grid**; **map column pinned right at 480 px**, `sticky top:72px`.
- [x] **Result card** (`coach-result-card.tsx`): 104/110 px arch thumb, name + italic terracotta "N km", discipline line, **"Based in X" / "Travels to X from Y"** line (from `travel_radius_km` + distance vs the coach's own area), 2-line clamped headline, attribute tag pills. Active card gets a terracotta border when its pin is selected; hovering a card selects its pin.
- [x] **Map** (`coach-map.tsx`, client, dynamic-imported with SSR off): MapLibre GL + OpenFreeMap vector tiles with a custom style (cream ground, tint roads, ink labels), dashed terracotta radius circle, pulsing `ring` origin dot, one **"N km" pill marker per coach** (ink, terracotta when active, `pop` on mount), active-coach card overlay at the bottom (`fade`). Mobile map view is the full frame below the header. Mock coaches have real lat/long already so they plot.
- [x] Radius change re-queries in place (URL param `r`, default 50) and animates the circle.
- [x] The three SEO listing routes swap their card component for the new result card and adopt the light header variant; they do **not** get the map or chips (they are static, indexable pages — keep them simple).
- [x] `search-facets.tsx` and `pill-dropdown.tsx` retired once nothing imports them.

**Done 10 Sep 2026.** `/search` rebuilt: ink header with the summary pill (phones stack the pill + a chip rail inside the sticky bar), six URL-driven chips (`a=` attribute terms + `new=1` for taking-students), a 25–150 km radius slider that commits on release (`RadiusSlider`), `CoachResultCard` with the "Based in / Travels to … from …" line, the "Nobody quite right?" card, the floating Map/List toggle, and `CoachMap` — MapLibre GL on OpenFreeMap vector tiles recoloured into the palette at load, a dashed terracotta radius circle, pulsing origin dot, "N km" pill pins keyed by slug, hover-card ↔ pin selection, the selected coach's card over the map. `searchCoaches` now returns lat/long/taking-students/radius. Listing routes use the result card. **The map was blank for a long time and the cause is worth recording:** MapLibre parses tiles in a Web Worker whose default URL comes from `import.meta.url`; under Turbopack that isn't `http(s):`, so the URL resolved to `""` — the page itself — and every tile sat at "loading" forever with no error. Fix: `scripts/copy-maplibre-worker.mjs` (runs as `predev`/`prebuild`) copies MapLibre's worker + shared chunk into `public/vendor/` (gitignored) and `setWorkerUrl("/vendor/…")` is called **immediately before `new Map`** — a module-scope call was evaluated against a different module instance and silently never applied. Also: the desktop map column is mounted only when `useWide()` (a `useSyncExternalStore` media query) says so, so phones don't run a hidden second map; MapLibre errors are logged; a dev-only `window.__ecaMap` handle exists for the harness. Measured: **49/49** assertions at 1280, **30/30** at 390 (`assertions/r4-search-*.json`); `search-behaviour.mjs` all green — h1 count = cards = `/api/coach-count`; chips toggle URL params and narrow/restore counts; taking-students narrows; slider writes `r=` and counts are monotonic 25 ≤ 50 ≤ 150; one pin per result, origin dot, 47 tile/style responses all 200, canvas mounted; hover selects the pin, clicking another coach's pin moves the selection and the overlay card follows; chips/slider/cards/pins keyboard-reachable, slider labelled; phone chips in the header, toggle pinned 18px above the fold, map view mounts exactly one map with pins, back to list keeps URL, no horizontal scroll. Harness lessons: strict `networkidle` never arrives on a page with a live map (now `load` + bounded idle); the phone/desktop twin of a control must be selected by visibility; headless GPU chatter filtered from console errors.
...
- [x] **Desktop**: light header with "← Back to results"; `1fr 400px` grid; 300 px arch portrait beside the name block (name on two lines at 68 px), "Save to favourites" outlined pill; headline 32 px; two-column skills+qualifications / setup; ink testimonials card 3-up; clinic row with "Details →"; **sticky aside** enquiry card (`top:96px`, shadow) with "Prefer to call?" + reveal.
- [x] **Enquiry form** (`contact-form.tsx` rewritten, shared by sheet and aside): Your name, Email or mobile, want chips (Regular lessons / One-off / Clinic — single select), message, "Send enquiry", "Goes straight to Name. We never share your details." Success state replaces the form in place. Disabled with the canvas's copy when `taking_students = 'no'`.
- [x] Pronouns: the canvas says "What she helps with" / "From her riders". Add `coach_profiles.pronoun_set` is **not** in §2 on purpose — use the coach's name ("What Isabella helps with", "From Isabella's riders") so nothing is guessed.
- [x] Structured data (`Person`, breadcrumbs) untouched; `knowsAbout` gains nothing new.

**Done 10 Sep 2026.** One DOM, CSS-switched between the frames: phones get a 440px full-bleed photo (`kb 10s`) fading into the cream, the page's own "← Results / ♡" bar over it (the site header hides below 768px on coach routes), name-block stack, skills/setup/qualifications, the ink testimonials band as a snap rail, the next clinic card, "Get in touch" with click-to-reveal, and the sticky "Enquire with Name · Free · no account needed" bar opening a bottom sheet (`sheet .45s`, scrim `fade .3s`, focus in/out, Escape, body scroll lock); desktop gets the light header with "← Back to results", the 300px arch portrait beside a two-line name, "Save to favourites" pill, two-column skills+qualifications / setup, a 3-up testimonials card, the clinic row with "Details →", and the sticky enquiry aside with "Prefer to call?". The enquiry form (name, email-or-mobile, regular/one-off/clinic chips, message) is shared by sheet and aside. **The phone number never reaches the HTML** — `PhoneReveal` fetches it on click and logs a `reveal` event. Closed coaches (`taking_students = 'no'`) get the disabled bar and an explanation in the aside instead of a form (a real bug caught by the smoke test: the first cut branched only on the form toggle). Headings say "What Isabella helps with" / "From Isabella's riders" — the coach's name, never a guessed pronoun. Measured: **57/57** assertions at 390 and **56/56** at 1280 (`assertions/r5-profile-*.json`); `scripts/smoke/r5-profile.mjs` all green with a throwaway coach + rider: number absent from server HTML, status/where-line/skills/testimonials/clinic rendered, phone bar pinned while scrolled, sheet opens with the `sheet` keyframe, scroll locked, focus into the title and back to the bar, Escape closes; reveal shows the real number and writes one `reveal` row; enquiry from the sheet (want=clinic, mobile contact) and from the aside (want=regular) each write a `status=new` row; aside sticky; three testimonial cards; logged-in rider's heart flips to "Saved" without reload and a `favourites` row exists; closed coach → disabled bar, pill copy, no form, no new rows; mock coach renders every section with no phone in the HTML; throwaways cascaded away. Harness lessons: check rendered `innerText`, not raw HTML (React streams comment nodes between expressions, and `innerText` applies `text-transform`); full-page screenshots draw sticky-bottom bars mid-page (stitching artefact — the scrolled-position check is the real one); Tailwind's `rounded-full` is not 999px.
...
- [x] Hero: full-bleed photo (canvas uses Unsplash `photo-1598974357801-cbca100e65d3`; download the master to `assets/hero/for-coaches.jpg` and run the ladder script — Unsplash licence, same as the mock set), 700/720 px, `kb`, parallax, canvas scrims, eyebrow, **seven `rise` words with "exactly" italic peach**, lead, two CTAs (terracotta "Join as a founding coach" → `/signup?role=coach`, outlined "See what's included" → `#included`).
- [x] Uppercase marquee strip with peach dots (`45s/60s`).
- [x] Founding section: deep-ink with the second Unsplash photo at `.35/.4` opacity + gradient; desktop pairs it with the monthly-email card in a 2-col grid.
- [x] Monthly email card restyled (`monthly-email-example.tsx`): shade header row, subject, 2×2 / 4-up stat tiles, body copy, shade footer.
- [x] "What we actually do" ink section: sticky left heading on desktop, numbered italic-peach list.
- [x] Plans: Monthly/Yearly toggle pill (`Yearly · save two months` on desktop), three tier cards (Spotlight featured = ink card, "Most popular" peach label, terracotta button; others cream with ink outline button), desktop hover `translateY(-6px)`. Prices from the tier constants (until the `settings` table exists — noted in §6).
- [x] Comparison: desktop full table with the Spotlight column tinted; mobile collapses it behind "Full comparison, all N features" (+/−).
- [x] "We never take a *percentage* of a lesson." band. FAQ with the `Accordion` restyled to the round +/− control, one open at a time, first open by default; "Tell us what to fix" shade card (sticky left on desktop).

**Done 10 Sep 2026.** `/for-coaches` rebuilt to the 2a/2b frames: full-bleed hero on the chestnut portrait (Unsplash master square-cropped by the ladder script so `object-position` still has room; 700/720px tall; seven `rise` words with "exactly" italic peach; two soft-radius CTAs), the uppercase promise ticker (45s/60s), founding band/card with the **pony-club** photo behind a wash — the canvas's second Unsplash horse showed visible ribs, which a horse-savvy audience reads as underweight, so it was swapped for an already-vetted shot — the monthly email card, the sticky-left "What we actually do" ink section, `Plans` (Monthly/Yearly toggle, ink Spotlight card, desktop table / phone fold), the no-commission band, and the FAQ with the restyled `Accordion` (round +/−, first open, one at a time) plus the feedback card. Header floats over this route now (`OVERLAY_ROUTES`, layout script). `scripts/build-hero-images.mjs` gained per-crop `aspect`/`position` and no longer wipes `public/hero`. Measured: **68/68** at 390 and **51/51** at 1280; `for-coaches-behaviour.mjs` all green at both widths (seven words with 70ms-stepped delays, ticker, every price and alt line flips with the toggle, phone fold = header + 27 rows, desktop table 28 rows with 60 ticks, 8 FAQs with first open / one-at-a-time / aria-controls, ≥5 signup CTAs, "#included" scrolls, no overflow, no errors). Contrast: eyebrow 7.74/5.22, h1 11.52/6.53, lead 5.61/13.18, outline CTA 9.40, nav 13.74/9.58; "exactly" (peach, 92px) 3.94:1 — passes as large text, and the checker now applies WCAG's 3:1 large-text rule instead of a flat 4.5.
...
- [x] Light header variant with "View public profile" pill + avatar (coach's photo or initial) + first name.
- [x] Navigation: mobile is a horizontal tab row under the header (terracotta underline on the active tab, red badge with the new-enquiry count); desktop is a 200 px sticky left rail with a plan card and the "Tell Kim or Alana" note. Routes stay real (`/dashboard`, `/dashboard/enquiries`, `/dashboard/profile`, `/dashboard/clinics`, `/dashboard/billing`) — tabs are links, not client state, so deep links and back/forward work. `fade .5s` on each panel's content.
- [x] **Overview**: "August 2026 · your month so far" label, greeting ("Good month, Name." / "A quiet month, Name." chosen from the numbers), summary line built from real stats (top queries only once GSC is connected — until then the line uses reveals/enquiries deltas), four stat tiles with deltas, ink **12-month bar chart** (`grow .7s`, current month peach), "Taking new students?" segmented control (server action, optimistic), latest 3 enquiries, profile completeness bar + checklist with "Add" links, plan card. Desktop layout per the frame (`1.5fr 1fr` rows, "Next clinic" card).
- [x] **Enquiries**: mobile cards / desktop table; status pill cycles New → Replied → Booked → No response via a server action; "Reply by email" is a `mailto:` with the rider's contact and a subject line.
- [x] **Profile** (`profile-form.tsx` reshaped, actions unchanged where possible): sticky arch photo + "Change photo" (existing upload flow), Headline, Bio, discipline chips (keeps the R9f drag-to-reorder list beneath — the canvas's "first one leads your page title" note is the same rule), Based in (autocomplete) + Travels up to (km), "Contact riders see" **toggle switches**, Discard/Save. Skills, setup (+ detail), qualifications, testimonials and video sections keep their existing controls restyled with the new inputs — they are below the fold in the canvas's scroll, not absent.
- [x] **Clinics**: cards with the date block, "N places left", "N riders emailed", Edit / View page; "+ New clinic" terracotta pill; note line varies with tier.
- [x] **Billing**: ink plan card ("Founding coach" label, plan name, billing line, Next charge / Card tiles), Change plan option list (server action → Stripe/mock), Invoices + Update card (Customer Portal / mock), one-line cancel. Tier enum migration (`listed|spotlight|clinic`, old values mapped) lands here.
...
**Done 10 Sep 2026.** `/dashboard/*` rebuilt to the Dashboards 1a/1b frames. `loadDashboard()` (`src/lib/dashboard.ts`) gives the layout and every tab one context (coach row, tier, plan name/line, new-enquiry count); `DashboardNav` is a phone tab row (`.hs`, terracotta underline, red badge) and a desktop sticky rail with the plan card, both real routes. Header in dashboard mode: tagline "Coach dashboard", "View profile / View public profile" pill (coach's own slug), avatar with the coach's photo or initial, public nav and burger hidden. **Overview** renders one DOM: `data-month` label, greeting chosen from the numbers, summary line from real reveals/impressions, four `data-stat` tiles with `data-delta`, the ink 12-month `data-chart` (`grow .7s`, current month peach, bar heights from `twelveMonthViews`), `TakingStudentsControl` (server action, optimistic), `data-latest`, `data-completeness` (`profileCompleteness`, six items with "Add" links), phone plan card / desktop next-clinic card — phone and desktop orders come from grid `order-*`, never twin markup. **Enquiries**: phone cards / desktop table, `EnquiryStatusButton` cycles New → Replied → Booked → No response via `cycleEnquiryStatus`, `mailto:`/`tel:` by contact type. **Profile**: reshaped form (chips, `Switch` contact rows, Based in + Travels up to km, Years coaching, Discard/Save, "Preview as a rider" link); `saveProfile` persists `travel_radius_km`, `travels_to_rider`, `years_coaching`; video gated by `hasVideo(tier)`. **Clinics**: date-block cards with places left / riders emailed, `#new-clinic` form with capacity, Listed capped at one upcoming clinic (`clinicLimit`). **Billing**: ink plan card, `Change plan` list (`changePlan` → mock/Stripe), Invoices / Update card, one-line cancel. **Tier enum** migrated in `0019_tiers.sql` (`listed | spotlight | clinic`; `standard → listed`, `standard_plus_clinics → clinic`; enum values added outside the transaction; clinics RLS re-pointed to the tiers that can list them); `src/lib/tiers.ts` is the one place tier names, prices and capabilities live until the `settings` table exists; Stripe price IDs now read `NEXT_PUBLIC_STRIPE_PRICE_LISTED/SPOTLIGHT/CLINIC`. Measured: `scripts/smoke/r7-dashboard.mjs` **29/29** (stats equal SQL, bars 4→100% / 3→75%, badge = 2 new, completeness 50%, taking-students → DB row + public "Waitlist open", status cycle persisted, profile round-trip `{60, 7, travels, email}`, Listed cap hides the form, mock plan change writes `listed`, no page errors, cascade cleanup); parity **46/46** at 1280 and **30/30** at 390 on the overview (logged in as a seeded coach via `--login`; `parity-check.mjs` gained `absent: true` for "no visible match" — a hidden wrapper's children keep their own computed `display`, so checking `display: none` on the burger was the wrong question); enquiries / profile / clinics / billing captured at both widths and compared to the frames by eye (no assertion files — those frames are single states of the same components). Also found while wiring this up: `predev`/`prebuild` for the MapLibre worker copy had never been added to `package.json` (R4 relied on a manual copy), fixed here and `public/vendor/` ignored.
