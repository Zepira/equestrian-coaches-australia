...
- [x] **Phase 2 — Data + Auth:** live Supabase project connected (`.env` has real keys, gitignored — not committed) and all three migrations run successfully — schema, storage bucket, and discipline seed are live. Verified end-to-end via a real signup attempt against the running app. full schema + RLS in `supabase/migrations/0001_init.sql` (profiles, disciplines, coach_profiles w/ PostGIS location, coach_disciplines, coach_photos, testimonials, clinics with tier-gated RLS, favourites, rider_preferences, notifications_log, postcodes) + `supabase/seed.sql`. `src/lib/supabase/{client,server,middleware}.ts` set up per the official `@supabase/ssr` pattern; `src/proxy.ts` refreshes sessions and gates `/dashboard` (coach-only) + `/account` (auth-only). Login/signup pages wired to real `supabase.auth` calls (email confirmation flow via `/auth/callback`, sign-out route). **Everything degrades gracefully with no live project**: `isSupabaseConfigured` is false until `.env.local` has real Supabase keys, so proxy/header/forms all no-op or show a "not connected yet" notice instead of crashing — I cannot create the Supabase account/project myself (that's an account-creation action), so this needs you to create it and hand back the keys before phase 2 is functionally complete. See `.env.example` for what's needed.
...
- [x] **Phase 5 — Payments (code side done, blocked on a live Stripe account):** `src/lib/stripe.ts` (same `isStripeConfigured`-guarded pattern as Supabase), `src/app/dashboard/billing/actions.ts` (`startCheckout` creates/reuses a Stripe customer + Checkout session per tier; `openBillingPortal` sends an active coach to the hosted Customer Portal), `src/app/api/webhooks/stripe/route.ts` (verifies the signature, syncs `coach_profiles.subscription_tier/status/published` on `checkout.session.completed` / `customer.subscription.*` — uses the service-role client since Stripe calls with no user session). **Publishing only ever happens from the webhook**, never from the Checkout redirect, so cancelling out of Stripe can't grant a free listing. `/for-coaches` shows "Subscribe" (triggers Checkout directly) for a logged-in coach vs. "List your profile" (→ signup) for everyone else; `/dashboard` shows a "Complete your subscription" prompt when arriving with `?tier=`; `/dashboard/billing` shows real status and either plan-selection or portal buttons. Also fixed a real bug this phase surfaced: **login didn't redirect by role** (always went to `/account`) — now checks `profiles.role` when there's no explicit `?next=`. Verified via a real login smoke test (throwaway coach account, confirmed dashboard/billing/for-coaches all render correctly logged in, no console errors, then deleted). **Still needed:** a live Stripe account — create the two subscription Products/Prices ($9.99 "Standard", $14.95 "Standard + Clinics", both AUD monthly) and put `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `NEXT_PUBLIC_STRIPE_PRICE_STANDARD`, `NEXT_PUBLIC_STRIPE_PRICE_CLINICS` in `.env` (see `.env.example`) — I can't create the Stripe account myself, same as Supabase in phase 2. The webhook endpoint (`/api/webhooks/stripe`) needs registering in the Stripe dashboard (or via `stripe listen --forward-to` locally) once there's a URL to point it at.
