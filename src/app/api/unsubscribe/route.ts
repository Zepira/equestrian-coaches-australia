...
// (a=) or every alert (r=) stops at once. Only POST changes anything, so a
...
export async function POST(request: Request) {
  ...
  const result = await unsubscribe(service, { alert: url.searchParams.get("a") ?? undefined, rider: url.searchParams.get("r") ?? undefined });
