...
import { suppress } from "@/lib/audience";
...
export async function removeRider(formData: FormData) {
  ...
  // Their contact record goes too (its consents with it). The address stays
  // on the do-not-email list, the one thing kept, so nothing reaches them again.
  await service!.from("contacts").delete().eq("email", email);
  await suppress(service!, email, "admin", "deleted on request");
