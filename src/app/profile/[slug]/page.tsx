/**
 * A horse care professional's profile. /profile/[slug] is the planned home
 * for every profession's profiles (content/handbook/site-structure.html);
 * coaches are still at /coaches/[slug] until the route move. Laid out like
 * the coach profile (src/app/coaches/[slug]/page.tsx) and sharing its CSS
 * (`.coach-profile`), its enquiry form and its phone reveal, in the steel
 * accent (/profile is a horse care path, see isHorseCarePath).
 *
...
export function generateStaticParams() {
  return mockProfessionals.map((p) => ({ slug: p.slug }));
}
    ...
    <div className="coach-profile">
      <JsonLd
        data={breadcrumbSchema([
          ...
          { name: pro.name, url: `/profile/${pro.slug}` },
        ])}
      />
            ...
            <div className="relative -mt-14 px-[18px] wide:mt-0 wide:px-0 wide:pb-2">
              ...
              <div className="fade-in mt-3.5 flex flex-wrap gap-1.5" style={{ animationDuration: "0.7s", animationDelay: "0.15s" }}>
                ...
                {pro.specialities.map((s) => (
                  <span key={s} className="rounded-[var(--radius-pill)] border border-border bg-surface px-3 py-1.5 text-[13px] font-medium text-fg wide:px-[13px] wide:py-[7px]">
                    {s}
                  </span>
                ))}
              </div>
